...
# to delete stuff/ not for use
class Remove(views.APIView):
    def get(self, request, format=None):
        results = Endpoint.objects.filter(owner="Piotr")
        if results:
            result = results[0]
            result.delete()
...
class StopABTestView(views.APIView):
    def post(self, request, ab_test_id, format=None):
        ...
        return Response({"message": "AB Test finihsed", "summary": summary})